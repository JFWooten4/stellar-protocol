...
This annotation style came up in a recent conversation with Leigh at [stellar-docs#2705 (comment)](https://github.com/stellar/stellar-docs/pull/2705#discussion_r3667037648). We resolved that inline documentation was best practice since many people won't follow the Git blame. Separately, I've been migrating the meetings to stable reference URLs in the dev docs since last year.

CAP approval and network adoption were separate processes. The CAP Core Team determined whether the proposal should enter Final Comment Period and be accepted. After implementation, Stellar validators separately decided whether to activate the resulting protocol changes on the public network.

### 24-30 May 2019: Initial Proposal and Draft Status

- David Mazières created CAP-21 on 24 May 2019.
- The proposal was merged as a Draft on 30 May.
- Its initial public discussion was assigned to the `stellar-dev` mailing list.

### [13 Jun 2019](https://developers.stellar.org/meetings/2019/06/13): Preconditions and Fee-Bump Finalization

- The original preconditions proposal was presented following work with the Channels working group.
- CAP-21 was considered important for implementing payment channels.
- The group initially agreed to move CAP-21 into Final Comment Period for acceptance.

### [27 Jun 2019](https://developers.stellar.org/meetings/2019/06/27): Two-Part Payments and Preconditions

- CAP-21 returned from Final Comment Period to Draft.
- Its dependency on CAP-22, which concerned the effects of invalid transactions, blocked its acceptance.
- The group considered a separate implementation for payment-channel experiments.

### [25 Jul 2019](https://developers.stellar.org/meetings/2019/07/25): Path Payments and Preconditions Progress

- The discussion confirmed that CAP-21 depended on CAP-22.
- CAP-22 still required additional feedback to be addressed.
- David Mazières took responsibility for resolving the dependency.

### [5 Mar 2021](https://groups.google.com/g/stellar-dev/c/N8vzP2Mi89U/m/iiHD-4_fAAAJ): Public Review Reopened

- David Mazières revived CAP-21 with a substantially revised proposal.
- He identified payment channels, relative timelocks, account recovery, and less brittle presigned transactions as motivating uses.
- Nicolas Barry requested an acceptable payment-channel design and conformance with the current CAP template.
- Leigh McCulloch, Jonathan Jove, Nicolas Barry, and others conducted an extended public technical review on the mailing list.

### [11 Mar 2021](https://developers.stellar.org/meetings/2021/03/11): Generalized Preconditions and Payment-Channel Safeguards

This meeting provides the best conceptual overview of the revised CAP-21. It covers:

- Revisions following the introduction of claimable balances
- Relative timelocks and ledger gaps
- Transactions valid across sequence-number ranges
- `minSeqNum`, `minSeqAge`, and `minSeqLedgerGap`
- Payment-channel dispute and unilateral-recovery windows
- Invalidating stale transactions by advancing sequence numbers
- Reducing the brittleness of presigned transactions
- Account-recovery and transaction-queue-recovery use cases
- Same-ledger bump-sequence behavior
- Replay prevention and account recreation
- Stellar Core forwarding, validation, and implementation complexity
- Alternative designs involving claimable balances and external timelocks

### [29 Jul 2021](https://developers.stellar.org/meetings/2021/07/29): Payment Channels on Stellar: Generalized Transaction Preconditions and Multisig

This detailed design and implementation review covers:

- Relative timelocks, ledger bounds, and extra signers
- Stateless hash-time-locked contract support
- CAP-40's dependency on CAP-21
- New failure modes when multiple preconditions fail
- Transactions that are "too early" rather than permanently invalid
- Account-entry extension structure and backward compatibility
- Behavior for accounts without the new extension
- Nested v. flattened XDR structures
- SDK and downstream-consumer implications
- Validation v. application of preconditions
- Revisions required before acceptance
- An acceptance vote was deferred pending evaluation of the effects on Horizon transaction submission, so CAP-21 remained a Draft.

### [9 Sep 2021](https://developers.stellar.org/meetings/2021/09/09): CAP-21: Generalized Transaction Preconditions for Payment Channels

This meeting primarily addresses implementation readiness:

- Whether sequence-modification tracking should be protocol-wide or opt-in
- Effects of account-entry extensions on historical Stellar Core tests
- Why an opt-in account flag was rejected
- Keeping timepoints unsigned
- Transaction forwarding and queue behavior
- Prioritizing transactions with sequence constraints
- Fee bidding and denial-of-service considerations
- Conflicts between sequence jumps and delay conditions
- First-seen handling of incompatible transactions
- Remaining specification changes required before acceptance
- Public specification review continued through 17 Sep, documenting the remaining questions about account extensions, historical tests, queue conflicts, sequence constraints, and validation semantics.

### [28 Oct 2021](https://developers.stellar.org/meetings/2021/10/28): Payment-Channel Readiness for CAP-0021

This was the final substantial readiness review. It covers:

- Same-ledger transaction invalidation through bump-sequence behavior
- Handling transactions that cannot coexist in one ledger
- Retaining the first incompatible transaction received
- Stellar Core overlay and testing requirements
- Horizon transaction-queue changes
- Expected SDK work
- Payment channels involving issued assets and trustlines
- Compatibility with regulated-asset and SEP-8 flows
- Account-recovery use cases
- Whether CAP-21 was ready for committee approval
- CAP-21 moved from `Draft` to `Awaiting Decision`.
- CAP-40, which supplied the complementary signed-payload signer for the payment-channel design, advanced alongside it.

### 4 Nov 2021: Additional Use Case Documented

- Leigh McCulloch documented a deterministic account-sequence-number use case identified through discussions with developers building on Stellar Turrets.
- The use case was added to CAP-21's design rationale while the proposal awaited a formal decision.

### [17 Feb 2022](https://developers.stellar.org/meetings/2022/02/17): SPEEDEX Lane Policy and Parameterization

- CAP-21 and CAP-40 were approved to enter Final Comment Period.
- Both proposals moved from `Awaiting Decision` to `FCP: Acceptance`.
- Both proposals targeted inclusion in Protocol 19.
- They were described as enabling payment channels, bridges, interoperability, and shared-signing workflows.
- CAP-21 received only a status update; the primary discussion concerned CAP-42 and CAP-44.

### 17-24 Feb 2022: Final Comment Period and Acceptance

- CAP-21 remained in `FCP: Acceptance` for one week, giving the community a final opportunity to identify objections.
- No concern was recorded that caused the proposal to return to Draft.
- CAP-21 and CAP-40 became `Accepted` on 24 Feb 2022.
- Acceptance approved the specifications but did not activate their changes on Stellar Mainnet.

### 17 Mar-29 Apr 2022: Post-Acceptance Implementation Alignment

- `TimePoint` remained unsigned, and a zero `maxLedger` was defined as no upper ledger bound.
- `Duration` was corrected from a signed to an unsigned 64-bit type.
- The [Stellar Core implementation of CAP-21 and CAP-40](https://github.com/stellar/stellar-core/commit/27c798e857a18593a0236b4ac587fc2f96958b08) merged on 13 Apr 2022.
- The inclusive and exclusive behavior of maximum time and ledger bounds, and the corresponding example, was clarified.
- These specification-maintenance changes aligned the accepted CAP with its implementation; they were not new acceptance votes.

### 9 May 2022: Protocol 19 Activated on Testnet

- Stellar Testnet upgraded to Protocol 19.
- The deployment allowed the ecosystem to exercise the CAP-21 and CAP-40 implementations before the public-network vote.
- SDF announced that the Mainnet validator vote would occur on 8 Jun 2022 at 15:00 UTC.

### 8 Jun 2022: Validator Vote and Mainnet Activation

- Stellar validators voted to accept Protocol 19.
- The network upgraded successfully at 15:00 UTC.
- The upgrade activated CAP-21's generalized transaction preconditions and CAP-40's signed-payload signer on Mainnet.
- CAP Core Team acceptance approved the proposal; validator assent made its rules part of the live Stellar protocol.
